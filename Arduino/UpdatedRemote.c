...
RF24 radio(8, 10);                             // (8,10) for Mini and (9,10) for UNO
RF24Network network(radio);

uint16_t this_node;                           // Our node address

const unsigned long interval = 60000; // ms       // Delay manager to send pings regularly.
...
void loop() {
  ...
  if ( now - last_time_sent >= interval ) {
    ...
    ok = send_P(to); // Base node sends the current active nodes out
...
bool send_P(uint16_t to)
{
  ...
  // Reading Soil Humidity

  output_value = analogRead(sensor_a_pin);
  output_value = map(output_value, 550, 0, 0, 100);
  Serial.print("Analog Mositure reading: ");
  Serial.print(output_value);

  payload.temperature = 41;
  payload.humidity = output_value;
  payload.light = 42;
  payload.fertility = 44;


  RF24NetworkHeader header(/*to node*/ to, /*type*/ 'P' /*Time*/);
  ...
  //printf("%d and the size of payload is: %d \n", payload.temperature, sizeof(payload));
  //printf("Value of ID: %f ,Temp : %f and humidity is %f respectively\n", payload.id, payload.temp, payload.humidity);
  printf("Values of the Payload: \n Temperature : %f \n Humidity : %f \n Light : %f and \n Fertility : %f \n",
         payload.temperature, payload.humidity, payload.light, payload.fertility);


  printf_P(PSTR("%lu: APP Sending %f to 0%d...\n\r"), millis(), payload.temperature, to);
